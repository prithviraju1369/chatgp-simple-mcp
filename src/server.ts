...
type TopMoversArgs = z.infer<typeof topMoversSchema>;
...
type TopMoversResponse = {
  top_gainers?: RawMoverRecord[];
  top_losers?: RawMoverRecord[];
  last_updated?: string;
  information?: string;
  note?: string;
};
...
const parseMovers = (records: RawMoverRecord[] = [], limit: number) =>
      ...
      contents: [
        {
          ...
          _meta: {
            'openai/widgetDescription':
              'Displays the latest Alpha Vantage top gainers and losers tables.',
...
mcpServer.registerTool(
  ...
  async (args: TopMoversArgs) => {
    ...
    const { data } = await axios.get<TopMoversResponse>(
...
app.get('/.well-known/apps.json', (req, res) => {
  ...
  res.json({
    ...
    description:
      'Browse current top stock market gainers and losers powered by Alpha Vantage directly inside ChatGPT.',
    instructions:
      'Help users explore Alpha Vantage top gainers and losers. Fetch fresh data with the topMovers tool and summarize noteworthy trends.',
    tools: [
      {
        type: 'mcp',
